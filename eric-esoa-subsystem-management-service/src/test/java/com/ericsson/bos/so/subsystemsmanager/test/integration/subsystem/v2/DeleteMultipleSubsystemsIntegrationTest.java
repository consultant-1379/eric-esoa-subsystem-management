...
public class DeleteMultipleSubsystemsIntegrationTest extends BaseIntegrationTest {
    ...
    public void persistSubsystems() {
        ...
        final ConnectionProperties connectionProperties = ConnectionPropertiesFactory
                .buildConnectionProperties(PropertyFactory.buildDefaultProperties());
    ...
    /**
     * GIVEN multiple subsystems not in use by active service WHEN delete subsystems THEN subsystems deleted.
     *
    ...
    /**
     * GIVEN multiple subsystems not exist WHEN delete subsystem THEN error response.
     *
     * @throws Exception the exception
     */
    @Test
    public void GIVEN_multipleSubsystemsNotExist_WHEN_deleteSubsystem_THEN_errorResponse() throws Exception {
        //Given
        final Long subsystemId1 = (long) 1;
        response = performMvcRequest(subsystemId1, primarySubsystem.getId());
        // then
        response.andExpect(MockMvcResultMatchers.status().isNotFound());
        checkResponseContainsInternalErrorCode("SSM-J-13");
        checkResponseContainsErrorData(subsystemId1.toString());
    }
    ...
    private ResultActions performMvcRequest(Long... subsystemIds) throws Exception {
        ...
        return mockMvc
                .perform(MockMvcRequestBuilders.delete(pathProperties.getV2().getSubsystemManagement().getBasePath() + "/" + Constants.SUBSYSTEMS)
                        .content(deleteRequestBody).contentType(MediaType.APPLICATION_JSON));
    }
