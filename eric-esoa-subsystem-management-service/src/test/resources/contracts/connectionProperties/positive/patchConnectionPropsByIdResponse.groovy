...
Contract.make {
    description(
        """
            ```
            when:
                A client makes a request to patch a connection properties in a subsystem
            then:
                the modified connection properties is returned
            ```
        """
        )
    request {
        method 'PATCH'
        url "/subsystem-manager/v1/subsystems/${value(consumer(anyPositiveInt()), producer('10'))}/connection-properties/${value(consumer(anyPositiveInt()), producer('2'))}"
        headers {
    ...
    response {
        ...
        body(file("PatchConnectionPropsByIdResponse.json").asString().replaceAll("%id", fromRequest().path(3).serverValue.toString()).replaceAll("%pid", fromRequest().path(5).serverValue.toString()))
        bodyMatchers {
