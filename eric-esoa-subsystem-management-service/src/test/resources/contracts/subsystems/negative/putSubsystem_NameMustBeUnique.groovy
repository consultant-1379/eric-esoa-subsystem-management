...
Contract.make {
    description(
        """
            ```
            when:
                A client makes a request to modify a specific subsystem with a name already in use'
            then:
            ...
            """
                {
                    ...
                    "subsystemType": "${value(consumer(optional(anyNonEmptyString())), producer('new-subsystem'))}",
