...
Contract.make {
    description(
        """
            ...
            when:
                A client makes a request to get the list of all subsystems filtered by param Map
            then:
    ...
    request {
        ...
        url ("/subsystem-manager/v1/subsystems") {
            queryParameters {
                parameter 'paramsMap': value(consumer(anyNonEmptyString()), producer(['operationalState=UNREACHABLE','subsystemType=PhysicalDevice']))
            }
        }
        headers {
