...
Contract.make {
    description(
        """
            ```
            when:
                A client makes a request to modify a specific connection properties in a nonexistent subsystem
            then:
    ...
    request {
        method 'PUT'
        url "/subsystem-manager/v2/subsystems/${value(consumer('11'), producer('11'))}/connection-properties/${value(consumer(anyPositiveInt()), producer('2'))}"
        headers {
